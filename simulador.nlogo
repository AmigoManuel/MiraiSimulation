...
globals[
  ...
  device_list
  ;; Cantidad de reinicios en dispositivos/routers
  resets_count
  ;; Lista de dispositivos anteriormente infectados
  fixed_devices
  ;; Dispositivos reinfectados
  reinfected_count
  ;; Reportes de vulnerables
  reports
]
...
to reset-procedure
  ...
  if moneda < prob_reset[
    set resets_count resets_count + 1
    set moneda random 2
    ;; 1 reinicia device
    if moneda = 1 [
      ask one-of devices[
        if color = red [
          set fixed_devices lput self fixed_devices
          set reports remove self reports
        ]
        set color green
      ]
    ]
    ;; 0 reinicia un router
    if moneda = 0 [
      ask one-of routers[
        if color = red [
          set fixed_devices lput self fixed_devices
